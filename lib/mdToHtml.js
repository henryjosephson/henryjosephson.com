...
// Add IDs to heading tags for internal linking
function addHeadingIds(htmlContent) {
  // Find all heading tags (h1-h6)
  return htmlContent.replace(/<(h[1-6])>(.*?)<\/\1>/g, (match, tag, content) => {
    // Generate ID from content: lowercase, replace spaces with hyphens, remove special chars
    const id = content
      .toLowerCase()
...
export async function markdownToHtml(markdown) {
  ...
  const result = await remark()
    .use(smartypants) // Process quotes and dashes
